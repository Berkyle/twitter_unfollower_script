    ...
    return new Promise((resolve, reject) => {
        ...
        if(fileCount == 0) {
            ...
            fs.readFile(lastEntry, (err, data) => {
                ...
                // If traitors exist, request info from Twitter API
...
const queryUsers = unfollowers => {
    return new Promise((resolve, reject) => {
        ...
        while(unfollowers.length) {
            let toQuery = "";
            let traitors = [];

            // Twitter only returns 100 users at a time
            if(unfollowers.length > 100)
                // Remove up to 100 users from the list of unfollowers
                toQuery = unfollowers.splice(0, 100).join(',');
            else
                toQuery = unfollowers.join(',');

            T.get('users/lookup', { user_id: toQuery }, (err, data, _) => {
                if(err[0].code === 17) 
                    resolve("Lost accounts were bots");
                else if(err) 
                    resolve(err);
                else
                    traitors.concat(data);
            });
        }

        resolve(summarize(traitors));
    });
};
